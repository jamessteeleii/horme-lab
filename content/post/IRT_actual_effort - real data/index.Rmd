...
But how does it work out when we introduce it to some real data? I got hold of some datasets where I was able to fit IRT models to the cognitive tasks performed, and where they had also captured some operationalisation of perception of effort.

### _Measurement scale issues_
Before I get to the actual datasets, we first need to talk about the issues with IRT models for my ideas about estimating effort as:
...
We'd expect to at the very least see this directional relationships between our IRT operationalisation of actual (objective) effort (i.e., $E_{A(ji,IRT)}$) and operationalisations of perception of (subjective) effort, or subjective effort.

So let's take a look at a couple of datasets.
...
##### - Update NASA model to use ordered beta regression. Cumuliative ordinal regression on probit assumes underlying     unbounded normal distributions, but the underlygin latent variable of perception of effort is conceptually bounded for us.

##### - So instead we use Kubinec's new package that overlays brms - https://cran.r-project.org/web/packages/ordbetareg/vignettes/package_introduction.html 
...
brm_SV_effort <- brm(SV_effort ~ irt_effort + (irt_effort | person),
                    data = comb_effort_SV,
                    family = brmsfamily("zero_one_inflated_beta", link = "logit"),
                    chains = 4,
                    iter = 3000, warmup = 1000,
                    cores = 4,
                    control = list(adapt_delta = 0.95), inits = 0
...
```{r,echo=FALSE,}
# 
#               # Person ability and AUC
#               comb_AUC <- left_join(rand_person, AUC_dat, by = "person")
# 
#               theta_AUC_plot <- comb_AUC %>%
#                 ggplot(aes(x=Estimate.Intercept, y=AUC)) +
#                 geom_hline(yintercept = 0, linetype = "dashed") +
#                 geom_vline(xintercept = 0, linetype = "dashed") +
#                 geom_point(alpha = 0.5) +
#                 geom_smooth(method = "lm", color = "Black") +
#                 # stat_regline_equation(label.x=-0.5, label.y=-Inf, vjust=-5, size=3) +
#                 labs(title = expression("Person Ability ("~theta~") and Effort Discounting AUC"),
#                      x = expression(theta~('ability on the logit scale estimated from perception of capacity')),
#                      y = "Effort Discounting AUC") +
#                 theme_classic()
# 
# 
#               (icc_p + theta_AUC_plot) / (NASA_effort_p + SV_effort_p)
...
# setwd("C:/Users/James/Dropbox/Research/Understanding Effort - and other perceptual phenomena/Horme Lab/Website/horme-lab/content/post/IRT_actual_effort - real data/datasets/Fleming et al., 2022")
# 
# fleming_dat <- read.csv("Fleming et al data for IRT.csv") 
# 
# fleming_dat <- rename(fleming_dat, person = subnum, item = nswitch)
# 
# fleming_dat <- fleming_dat %>%
#   mutate(person = as.factor(person),
#          item = as.factor(item)) %>%
#   group_by(person) %>%
#   mutate(avg_accuracy_person = mean(accuracy)) %>%
#   ungroup() %>%
#   group_by(person, item) %>%
#   mutate(avg_accuracy_item = mean(accuracy))
# 
# 
# 
# ## 1PL model
...
#                 theta = seq(from = -6, to = 6, length.out = 100)) %>% 
#   mutate(p = inv_logit_scaled(b_Intercept + theta - xi)) %>% 
#   group_by(theta, item) %>% 
#   summarise(p = mean(p)) 
# 
# icc_p <- post_icc %>% 
...
# 
# rand_item <- as.data.frame(rand$item)
# rand_item <- data.frame(beta = rep(rand_item$Estimate.Intercept, times = length(unique(fleming_dat$person)))) %>%
#   mutate(item = as.factor(rep(1:length(unique(fleming_dat$item)), times=length(unique(fleming_dat$person)))),
#          person = rep(rand_person$person, each=length(unique(fleming_dat$item)))) %>%
#   select(person, item, beta)
# 
# rand_all <- left_join(rand_person, rand_item, by = "person") %>%
#   rename(
...
#   )
# 
# comb_effort <- left_join(rand_all, fleming_dat, by = c("person", "item")) 
# 
# lmer_effort <- lmer(Effort ~ irt_effort + (irt_effort | person),
#                     data = comb_effort)
# 
# lmer_effort_pred <- ggeffects::ggpredict(lmer_effort, terms =  "irt_effort")
# 
# comb_effort_p <- comb_effort %>%
#   ggplot() +
#   geom_abline(intercept = 0, slope = 0.2, linetype = "dashed") +
#   # geom_point(aes(x = irt_effort*100, y = Effort), alpha = 0.5, size = 0.75) +
#   geom_smooth(aes(x = irt_effort*100, y = Effort, group=person), method="lm", alpha = 0.25, size = 0.05, color = "black", se = FALSE) +
#   geom_ribbon(data = lmer_effort_pred, aes(x=x*100, ymin=conf.low, ymax=conf.high), fill = "grey", alpha = 0.75) +
#   geom_line(data = lmer_effort_pred, aes(x=x*100, y=predicted), color = "black") +
#   scale_y_continuous(limits = c(0,20)) +
#   scale_x_continuous(limits = c(0,100)) +
#   labs(title = "IRT (1PL) Estimated Effort and NASA-TLX Effort",
#        y = "NASA-TLX Effort (0-10)",
#        x = expression("IRT Estimated Effort = exp("~beta~")/exp("~theta~")")) +
#   theme_classic()
# 
# comb_effort_p
